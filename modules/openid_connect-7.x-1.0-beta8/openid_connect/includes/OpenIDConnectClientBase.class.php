...
abstract class OpenIDConnectClientBase implements OpenIDConnectClientInterface {
  ...
  public function retrieveTokens($authorization_code) {
	...
	//watchdog('$response_data1', $response->data?$response->data:'null');
    if (!isset($response->error) && $response->code == 200) {
      //$response_data = drupal_json_decode($response->data);
	//mb_strlen("锘匡豢");  
	  $response_data = drupal_json_decode(ltrim($response->data,"锘匡豢"));
	  //watchdog('$response_data2', $response_data?$response_data:'null');
      $tokens = array(
