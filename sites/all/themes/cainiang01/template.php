...
function bartik_cainiang_process_node(&$variables) {
    //require_once DRUPAL_ROOT . '/sites/all/modules/comment_area/comment_area.module';
    if ($variables['view_mode'] == 'full' && node_is_page($variables['node'])) {
		$variables['nodecommentlike']=comment_area_nodecommentlikequery($variables['node']->nid);
		
		if(isset($variables['elements']['#JHProductOpti'])){
		    $products = $variables['elements']['#JHProductOpti'];
		    ...
		    if(is_array($products)){
			   ...
			   typeof(asycChangeValue)!='undefined'&&asycChangeValue(undefined);
			   });
...
function bartik_cainiang_preprocess_node(&$variables) {
  if ($variables['view_mode'] == 'full' && node_is_page($variables['node'])) {
	 ...
	 drupal_add_js(drupal_get_path('theme', 'bartik_cainiang') . '/slider/js/jquery.musliderEN-2.0.js',array( 'scope' => 'footer', 'weight' => 20));
	 drupal_add_js(drupal_get_path('theme', 'bartik_cainiang') . '/JS/Custom.js',array( 'scope' => 'footer', 'weight' => 115));
  ...
  if ($variables['node']->type == 'fruitseries' || $variables['node']->type == 'vegetableseries') 
{
    ...
    if($variables['view_mode'] == 'teaser' && isset($variables['view'])){
       //浠锋牸鏁版嵁
       $id = $variables['id'];
       $variables['product_price'] = ($variables['view']->result[$id-1]->commerce_price_amount)/100.0;
    }
    else if($variables['view_mode'] == 'teaser' && isset($variables['content']['product:commerce_price'])){
        $variables['product_price'] =   ($variables['content']['product:commerce_price']['#object']->commerce_price[und][0][amount])/100.0;
    }
  }
 ...
 * Override or insert variables into the block template.
 */
function bartik_cainiang_preprocess_jingheblock(&$variables) {
  // In the header region visually hide block titles.
  if ($variables['block']->region == 'header') {
    $variables['title_attributes_array']['class'][] = 'element-invisible';
  }
  $nodes=$variables['elements']['nodes'];
  $children_keys = element_children($nodes);
  $output = array();
  foreach ($children_keys as $key) {
    if (!empty($nodes[$key])) {
      $output[]= drupal_render($nodes[$key]);
    }
  }
...
function bartik_cainiang_preprocess_link(&$variables) {
    if(isset($variables['path'])&&strpos($variables['path'],'addressbook')!==false){
	$variables['theme_hook_suggestions'][] = 'link__shiping'; 
  }
}

function bartik_cainiang_preprocess_links__comment(&$variables) {
...
function bartik_cainiang_menu_local_action__shipping($variables) {
  ...
  $output = '<li><a class="jhaddaddress" href="/'.$link['href'].'" title='.'"澧炲姞鏀惰揣鍦板潃"'.'>';
...
function bartik_cainiang_form_element__bank($variables){
  ...
  switch ( $element['#id']) {
      case 'edit-commerce-payment-payment-method-alipay-f2frules-zhifubaosaoyisao':
         $output .= '<img class="pay-item-icon" for="edit-commerce-payment-payment-method-alipay-f2frules-zhifubaosaoyisao" alt="褰撻潰浠�" src=" https://zos.alipayobjects.com/rmsportal/iBUHPUBjQgjoXxBESctK.png">';
        break;
      case 'edit-commerce-payment-payment-method-alipay-pagerules-zhifubaowangye':
        $output .= '<img class="pay-item-icon" alt="鐢佃剳缃戠珯鏀粯"   src="/sites/all/themes/cainiang01/images/technology.png">';
        //$output .= '<img class="pay-item-icon" alt="鐢佃剳缃戠珯鏀粯" src=" https://zos.alipayobjects.com/rmsportal/RcCdEKSgiDeiFLXKTwOI.png">';
        break;
	  case 'edit-commerce-payment-payment-method-alipay-waprules-zhifubaoshouji':
	    $output .= '<img class="pay-item-icon" alt="APP鏀粯" src=" https://zos.alipayobjects.com/rmsportal/peUpMlQSBFhCpuKfSXSo.png">';
        //$output .= '<img class="pay-item-icon" alt="鎵嬫満缃戠珯鏀粯" src="https://zos.alipayobjects.com/rmsportal/AuBmpwXHMHBVjXzjcmNs.png">';
...
function bartik_cainiang_link__shiping($variables) {
	...
	if($variables['text']=='set as default'){
		$variables['text']='璁句负榛樿鍦板潃';
		$c='default';
		
	}
	elseif($variables['text']=='鍒犻櫎'){
		$c='delete';
...
function bartik_cainiang_field__taxonomy_term_reference($variables) {
  ...
  if (!$variables['label_hidden']) {
    $output .= '<dt><div class="field-label_JH">' . $variables['label'] . '</div></dt>';
  }

  // Render the items.
  $output .= ($variables['element']['#label_display'] == 'inline') ? '<dd><div class="field-items inline field-items_JH"><div class="field-item even">' : '<dd><div class="field-items field-items_JH"><div class="field-item even">';
  ...
  $output = '<dl class="dl-horizontal JHChangeValue">' . $output . '</dl>';
...
function bartik_cainiang_username($variables){
  ...
  //return '<a href=鈥漺tai://wp/mc;18503891489鈥�>Tel:18503891489</a>';
}
...
function bartik_cainiang_theme(){
   ...
   //create custom user_login.tpl.php
	...
	$items['jingheblock'] = array(
      'render element' => 'elements',
	  'path' => drupal_get_path('theme','bartik_cainiang').'/templates',
      'template' => 'jingheblock',
    );
	//dsm($items);
   return $items ;
}
...
function bartik_cainiang_preprocess_user_login(&$variables){
    $variables['form']['name']['#title']='鐢ㄦ埛璐﹀彿:';
    $variables['form']['pass']['#title']='鐢ㄦ埛瀵嗙爜:';
    $variables['form']['submit']['#value']='鐧诲綍';
